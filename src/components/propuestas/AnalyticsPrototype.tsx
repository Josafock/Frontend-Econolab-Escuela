...
function Regression() {
  const [category, setCategory] = useState("Perfil clínico"); const [parameters, setParameters] = useState(12); const [urgent, setUrgent] = useState(false);
  const result = useMemo(() => ({ price: Math.round(250 + parameters * 36 + (category === "Estudio especializado" ? 410 : 0) + (urgent ? 180 : 0)), hours: Math.max(4, Math.round(parameters * 1.6 + (category === "Estudio especializado" ? 18 : 0) - (urgent ? 8 : 0))) }), [category, parameters, urgent]);
  return <Shell proposal="regresion"><Heading eyebrow="Propuesta 2 · Regresión" title="Estimación de precio" description="Simula el registro de un nuevo estudio y propone valores iniciales a partir de estudios similares del catálogo antes de su validación administrativa." icon={TrendingUp} />
    <div className="grid gap-5 lg:grid-cols-[.9fr_1.25fr]"><Card className="p-6"><div className="mb-5 flex items-center gap-3"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-red-50 text-red-600"><FlaskConical size={20} /></span><div><h2 className="font-semibold">Características del nuevo estudio</h2><p className="text-sm text-slate-500">Completa la información disponible</p></div></div><div className="space-y-4"><label className="block text-sm font-medium">Nombre del estudio<input defaultValue="Perfil metabólico avanzado" className="mt-2 w-full rounded-xl border border-slate-200 px-4 py-3 text-sm outline-none focus:border-red-300" /></label><label className="block text-sm font-medium">Categoría<select value={category} onChange={(e)=>setCategory(e.target.value)} className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm"><option>Perfil clínico</option><option>Estudio especializado</option><option>Hormonal</option></select></label><label className="block text-sm font-medium">Número de parámetros<div className="mt-2 flex items-center gap-4"><input type="range" min="2" max="30" value={parameters} onChange={(e)=>setParameters(Number(e.target.value))} className="w-full accent-red-600"/><b className="w-10 rounded-lg bg-slate-100 py-2 text-center">{parameters}</b></div></label><label className="block text-sm font-medium">Método de análisis<select className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-4 py-3 text-sm"><option>Química seca</option><option>Inmunoensayo</option><option>Espectrofotometría</option></select></label><button onClick={()=>setUrgent(!urgent)} className={`flex w-full items-center justify-between rounded-xl border p-3 text-left text-sm ${urgent ? "border-red-300 bg-red-50" : "border-slate-200"}`}><span><b className="block">Entrega prioritaria</b><span className="text-xs text-slate-500">Considerar procesamiento urgente</span></span><span className={`h-6 w-11 rounded-full p-1 ${urgent ? "bg-red-600" : "bg-slate-200"}`}><span className={`block h-4 w-4 rounded-full bg-white transition ${urgent ? "translate-x-5" : ""}`} /></span></button></div></Card>
      <div className="space-y-5"><Card className="overflow-hidden"><div className="border-b border-slate-100 bg-gradient-to-r from-red-50 to-white p-6"><div className="flex items-center gap-2 text-sm font-semibold text-red-700"><Sparkles size={16}/> Estimación generada por el modelo</div><div className="mt-5 grid grid-cols-2 gap-4"><div><p className="text-sm text-slate-500">Precio sugerido</p><p className="mt-1 text-4xl font-semibold tracking-tight">${result.price.toLocaleString("es-MX")} <span className="text-sm font-normal text-slate-400">MXN</span></p><p className="mt-2 text-xs text-emerald-600">Rango recomendado ± $95</p></div><div className="border-l border-red-100 pl-6"><p className="text-sm text-slate-500">Tiempo estimado</p><p className="mt-1 text-4xl font-semibold tracking-tight">{result.hours} <span className="text-sm font-normal text-slate-400">horas</span></p><p className="mt-2 text-xs text-emerald-600">Confianza del modelo: 91%</p></div></div></div><div className="p-6"><div className="flex items-center justify-between"><h3 className="font-semibold">Comparación con estudios similares</h3><button className="text-xs font-semibold text-red-600">Ver catálogo completo</button></div><div className="mt-4 overflow-hidden rounded-xl border border-slate-200"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr><th className="p-3">Estudio</th><th>Parámetros</th><th>Precio</th><th>Entrega</th></tr></thead><tbody>{[["Perfil hepático",9,"$610","12 h"],["Química sanguínea 12",12,"$720","18 h"],["Perfil metabólico",14,"$845","24 h"]].map((r)=><tr key={String(r[0])} className="border-t border-slate-100"><td className="p-3 font-medium">{r[0]}</td><td>{r[1]}</td><td>{r[2]}</td><td>{r[3]}</td></tr>)}</tbody></table></div></div></Card><div className="grid gap-4 sm:grid-cols-3">{[[CircleDollarSign,"Ahorro estimado","12%","vs. definición manual"],[Clock3,"Tiempo de captura","2 min","por nuevo estudio"],[Activity,"Datos utilizados","86","estudios históricos"]].map(([Icon,label,val,hint])=><Card key={String(label)} className="p-4"><Icon className="text-red-600" size={20}/><p className="mt-3 text-xs text-slate-500">{String(label)}</p><p className="text-xl font-semibold">{String(val)}</p><p className="text-[11px] text-slate-400">{String(hint)}</p></Card>)}</div></div>
    </div></Shell>;
}
