...
export default function ValidateTokenForm({ setIsValidToken, token, setToken }: ValidateTokenProps) {
  ...
  const handleResend = () => {
    if (cooldown > 0) return;
    toast("Si tu cuenta existe, te reenviamos un nuevo codigo.");
    setCooldown(30);
  };
          ...
          <div className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm ring-1 ring-black/5">
            <div className="flex items-center gap-3 border-b border-gray-100 px-8 py-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500">Introduce el codigo de 6 digitos que enviamos a tu correo</p>
              </div>
            </div>
            ...
            <div className="grid gap-6 px-6 py-6 sm:grid-cols-5 sm:px-8 sm:py-8">
              ...
              <div className="rounded-lg border border-gray-200 bg-gray-50 p-4 sm:col-span-2">
                ...
                <ul className="list-disc space-y-2 pl-5 text-xs text-gray-600">
                  ...
                  <li>Evita pegar con espacios ocultos; escribe los 6 digitos manualmente.</li>
                  <li>El codigo expira por seguridad. Si caduca, solicita uno nuevo.</li>
                </ul>
                <p className="mt-3 text-xs text-gray-600">
                  ¿No tienes acceso al correo? Vuelve a <Link href="/auth/register" className="text-red-600 underline underline-offset-2 hover:text-red-700">registro</Link> y usa otra direccion.
                </p>
              </div>
            </div>
