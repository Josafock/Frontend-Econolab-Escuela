...
export default function CatalogExcelManager<Row extends Record<string, string>>({
  ...
  const handleExport = async () => {
    if (exportedRows.length === 0) {
      toast(`No hay ${entityLabel} para exportar en este momento.`);
      return;
    }
  ...
  const handleFilesSelected = async (files: FileList | null) => {
    ...
    try {
      ...
      if (!imported.length) {
        toast('El archivo no contenia filas con datos.');
      } else {
        toast.success(
  ...
  const handleImport = async () => {
    ...
    if (!importableRows.length) {
      toast('Corrige o elimina las filas con errores antes de importar.');
      return;
    }
